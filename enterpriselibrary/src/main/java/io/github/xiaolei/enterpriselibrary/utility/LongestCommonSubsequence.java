 ...
 * https://en.wikibooks.org/wiki/Algorithm_Implementation/Strings/Longest_common_subsequence#Java
 *
 * @author jhess
 */
public abstract class LongestCommonSubsequence<VALUE> {
    ...
    private void compareMatched(int i, int j) {
        mCorrectWordCount = 0;
        calculateLcs();

        while (!(i == 0 && j == 0)) {
            if (i > 0 && j > 0 && isXYEqual(i, j)) {
                this.diff.add(new DiffEntry<VALUE>(DiffType.MATCH,
                        valueOfXInternal(i), i - 1));
                mCorrectWordCount++;
                i--;
                j--;
    ...
    public static enum DiffType {
        ADD("+", "add"), REMOVE("-", "remove"), MATCH(" ", "match");
    ...
    public static class DiffEntry<VALUE> {
        ...
        public DiffEntry(DiffType type, VALUE value, int index) {
            super();
            this.type = type;
            this.value = value;
            this.index = index;
